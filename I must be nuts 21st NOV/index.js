...
//var xhr = new XMLHttpRequest();//require('xhr');
...
var th = require('./townhall');
var va = require('./virtualassistant');
...
//writing a node-gtts function
function nodegttsfunction(req,res){
  router.get('/', function(req, res) {
    res.set({'Content-Type': 'audio/mpeg'});
    console.log("contenttype is set. now piping to res.")
...
/*
//writing a pollytts node package implementation
function pollyfunction(req,res){
...
//end of polly block
*/
function ttsfunction(req,res){
  console.log(req.body);
  var ttsvar = JSON.stringify(req.body.text);
  googleTTS(ttsvar, 'hi', 1)   // speed normal = 1 (default), slow = 0.24
  .then(function (url) {
    console.log(url); // https://translate.google.com/translate_tts?...
    file = fs.createWriteStream("file.mp3");
    var request = https.get(url, function(response) {
     
      response.pipe(file); 
      bufferaudio = response.socket._tlsOptions.session;
...
/*
function ttswavfunction(url,dest){
...
app.post('/tts',function(req,res){
  //Synth.Synthesize(req,res);
  ttsfunction(req,res);
  //pollyfunction(req,res);
  //nodegttsfunction(req,res);
});
...
app.post("/mlTataaia", function(req, res) {
  console.log("Hit the mltataaia bot");
  tataAia.mlBot(req, res);
});

app.post("/townhall", function(req, res) {
  th.formatTime(req, res);
});

app.post("/va", function(req, res) {
  console.log("i am hitting post request right");
  va.vabot(req, res);
});

app.post("/form", function(req, res) {
  console.log("i am hitting forms endpoint route");
  console.log("stringify reques");
});
